...
while(!convergence){
  ...
  if (max(gelman_output$psrf[,2])< 1.1){
    ...
    names(result) =c("qu","au","tau","d", "gu",
                     "lambda", "catch_prop", "phi", "n", "X",
                     "c", "b", "rho", "alpha", "g_mult",
                     "q_mult")
    ...
    names(result) =c("qu","au","tau","d", "gu",
                     "lambda", "catch_prop", "phi", "n", "X",
                     "c", "b", "rho", "a_mult", "g_mult",
                     "q_mult")
    
    save(result, file="InterventionOnly_NoAspiration.RData")
    
    total_iterations_so_far = total_iterations_so_far + 1000000
