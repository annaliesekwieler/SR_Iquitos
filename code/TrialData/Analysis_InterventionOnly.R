...
png("../../results/ManuscriptFigures/Supplement/Non_SR_Effects_Parity.png",
    height=8.5, width=11, units = "in", pointsize = 12, res = 72)

par(mfrow=c(3, 4),  cex=1.2, mar=c(2, 3, 3, 1))


plot(density(result$qu), main="Exit Rate, Untreated", xlab="", xlim=c(0, 2.5), ylim=c(0, 5))
...
plot(density(result$au), main="Biting Rate, Untreated",
...
plot(density(result$lambda), main="Mosquito Emergence",
     xlab="", xlim=c(0, .2), ylim=c(0,75))
...
plot(density(result$phi), main="Dispersion, Aspiration",
     xlab="", xlim=c(.15, .18),)
...
png("../../results/ManuscriptFigures/Supplement/SR_Effects_Parity.png",
    height=8.5,
...
png("../../results/ManuscriptFigures/Supplement/PPD_parity.png", height=8.5,
    width=11, units = "in", pointsize = 12, res = 72)

par(mfrow=c(5, 3), mar=c(1, 5, 1, 2), cex=1.2)

boxplot(prop_parous_int_control_ppd, main="Control", ylim=c(.75, .81), ylab="Parity")
...
boxplot(prop_parous_int_treatment_ppd, main="Treatment", ylim=c(.75, .81))
...
boxplot(prop_parous_int_treatment_ppd, main="Untreated", ylim=c(.75, .81))
...
boxplot(prop_sc_control, ylab="Seroconversions", ylim=c(0.1, .4))
...
png("../../results/ManuscriptFigures/Supplement/Correlations_Parity.png",height=8.5,
    width=11, units = "in", pointsize = 12, res = 72)
...
pairs(result %>%
        rename(Mortality = gu, Biting = au, Digestion = d, "Prop'n Caught" = catch_prop, Repellency = rho,
               "Exit Effect" = q_mult, "Biting Effect" = a_mult, "Mortality" = gu, "Trans. Prob" = b) %>%
        dplyr::select(Mortality, Biting, Digestion, "Prop'n Caught", Repellency, "Exit Effect", 
                      "Biting Effect", "Mortality", "Trans. Prob"))
dev.off()
...
result = result[sample(1:nrow(result), size = 500, replace = T),]
...
png("../../results/ManuscriptFigures/Supplement/Coverage_Parity.png",height=8.5,
    width=11, units = "in", pointsize = 12, res = 72)

par(mfrow=c(1,1), mar=c(4,5,3,4), cex=1.5)
plot(direct_effect ~ coverages, type="l", ylab="Effect Size",
     xlab="Proportion Covered", lwd=3, ylim=c(-4, .6))
